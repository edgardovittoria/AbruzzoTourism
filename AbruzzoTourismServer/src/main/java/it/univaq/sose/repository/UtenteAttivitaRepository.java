...
public class UtenteAttivitaRepository {
	...
	public UtenteAttivita getUtenteAttivitaFromID(int ID) {
		UtenteAttivita utenteAttivita = (UtenteAttivita) this.em
				.createQuery("select u from UtenteAttivita u  where IDUtenteAttivita=" + ID).getResultList().get(0);
		return utenteAttivita;
	}

	public UtenteAttivita getUtenteAttivitaFromEmail(String email) {
		UtenteAttivita utenteAttivita = (UtenteAttivita) this.em
				.createQuery("select u from UtenteAttivita u  where email LIKE :email").setParameter("email", email)
				.getResultList().get(0);
		return utenteAttivita;
	}
	...
	public boolean loginUtenteAttivita(UtenteAttivita utenteAttivita) {
		try {
			UtenteAttivita utenteAttivitaresult = (UtenteAttivita) this.em
					.createQuery("select u from UtenteAttivita u  where email LIKE :email")
					.setParameter("email", utenteAttivita.getEmail()).getResultList().get(0);
